...
Chat IDs are opaque to the orchestrator. Optional methods on `Channel` provide `groupProjectChat`, `createProjectChat`, and `closeProjectChat`. Grouping is a separate step from chat creation: the orchestrator awaits `groupProjectChat` after a chat is created, connected, or adopted as a Project main, and `createProjectChat` returns a chat without organizing it. Closure includes its own removal. There is no nested lifecycle object or event hook. Each platform uses a full conversation surface: Telegram topics and Slack channels. Mapping every platform to message threads would weaken the independent-workspace experience.
...
Sections are Slack-specific and stay inside the Slack adapter. Grouping changes belong to lifecycle operations. `groupProjectChat` attaches one channel to its Project's section and creates the section, with its initial users, when the Project has none. Archiving removes only the archived workspace's channel, including pre-existing main channels. Spawn rollback also detaches its created chat. Startup and ordinary turns do not synchronize grouping.

Attaching creates a missing section rather than skipping, because a Project's section is not guaranteed to exist when its first peer arrives: the Project may predate sections, its main may be a DM, or an earlier attempt may have failed. Only `remove` treats a missing section as nothing to do. A section the user deleted is recognized by its `date_delete` and left alone, so create-on-attach never resurrects one.

Slack reads the live group before each change and sends only the changed channel list. Manual names, handles, members, unrelated channels, and disabled state remain untouched. An empty group is not automatically disabled. The ownership marker lets a manually renamed group remain identifiable; removing the marker relinquishes that ownership, and a later attach then creates a second, ClearClaw-owned group under a hashed handle.

A Slack DM cannot belong to a sidebar section, so a Project whose main is a DM gets its section from the first channel peer that attaches rather than from the main.

Section failures are best-effort and do not fail the peer lifecycle, but they are no longer silent: `groupProjectChat` returns a note naming the Slack error and, for the three that no code change can fix (`paid_teams_only`, `missing_scope`, `permission_denied`), the setting to change. The orchestrator appends that note to the reply for the operation that triggered it. Removal failures stay log-only. Removing automatic reconciliation means failed grouping is not repaired at startup; a Project that drifted before this behavior existed stays drifted until a lifecycle operation touches it. Section updates use a direct read and write without a queue. Slack's [update API](https://docs.slack.dev/reference/methods/usergroups.update/) replaces the channel list, so overlapping lifecycle operations or manual edits can overwrite each other's grouping changes. We accept this sidebar inconsistency to keep the lifecycle simple; peer creation and operation do not depend on grouping success. Completed manual edits are preserved when read before the update.
