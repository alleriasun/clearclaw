...
- [x] Slack `createChat`/`closeChat` — private channels via `conversations.create`/`invite`/`archive` with `groups:write`; flat channels, no container tier (follow-on to 1c)
