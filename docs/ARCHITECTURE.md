...
- **Model:** `/model <name>` sets a per-workspace model override for Claude Code or Codex, applied on the next turn; `/model` alone shows the configured override. Codex requires the ACP model-selection adapter support. `/model default` clears the override and keeps the current session; `/new` starts a session using the engine's configured default. Resumed sessions may retain their previously selected model. The session ID is captured from the engine's first message so cancellation does not lose it. A reported model belongs to usage stats and never becomes a user override.
