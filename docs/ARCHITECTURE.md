...
`workspace_create` is the single creation operation, available in every connected workspace conversation. The peer joins the caller's project by default. A `project` naming an existing project joins it; any other name creates that project with this peer as its main, so a project's name is independent of its main workspace's name. It supports automatic platform chat creation and manual binding. `cwd` is required and must already exist; ClearClaw never creates or deletes a workspace directory.
...
Both adapters expose `createProjectChat` and `closeProjectChat` directly on `Channel`. Slack also implements `groupProjectChat`, awaited after a chat is created, connected, or adopted as a Project main; it attaches that channel to the Project's sidebar section and creates the section when the Project has none. Telegram does not implement it, and that is correct rather than missing parity: a topic's grouping is the forum it lives in, so there is nothing to reassign. Slack section handling stays inside the adapter: closure archives the channel and removes it from the live User Group. Completed manual section edits are preserved. Grouping never fails a lifecycle operation, but `groupProjectChat` returns a note when it did not happen, and the orchestrator shows it. Telegram validates topic support before creation and closes topics; whole-chat closure is unsupported. Workspace archive closes the bound chat regardless of origin before removing its binding, and reports closure failures without unbinding. Directory ownership remains a separate cleanup decision. There is no startup or per-turn grouping synchronization.
...
- `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` — Slack bot + app-level token for Socket Mode. If both Slack and Telegram tokens are set, Slack takes priority.
  ...
  - Slack groups a Project's channels into a shared sidebar section, initially named after the Project, with handle `cc-<project-slug>` (plus a stable hash suffix when that handle is occupied). The section is created by the first channel that attaches to a Project without one, so Projects whose main is a DM are grouped by their peers. ClearClaw marks the User Groups it creates and never updates or disables an unmarked group; a section you delete stays deleted. This requires a paid plan, `usergroups:read` and `usergroups:write`, and workspace User Group permissions that allow everyone to manage groups. When any of those is missing, ClearClaw reports the Slack error and the setting to change instead of grouping silently. Archive removes its channel. Existing group names, members, other channels, and disabled state are preserved.
