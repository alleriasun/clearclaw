...
import { Bot, GrammyError, type Context, InlineKeyboard, InputFile } from "grammy";
...
export class TelegramChannel extends EventEmitter implements Channel {
  ...
  async closeProjectChat(chatId: string): Promise<void> {
    ...
    if (!thread) throw new Error("Telegram cannot close a whole chat; only topics can be archived");
    ...
    try {
      ...
      if (err instanceof GrammyError && err.description === "Bad Request: TOPIC_NOT_MODIFIED") return;
      throw err;
    }
  }

  private async validateProjectChat(projectName: string, anchor: string): Promise<void> {
    const parentId = this.numericId(anchor);
    if (!Number.isFinite(parentId)) {
      throw new Error(`Telegram project "${projectName}" must use a valid chat ID`);
    }

    const chat = await this.bot.api.getChat(parentId);
