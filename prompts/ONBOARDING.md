...
2. **Ask what they want to work on.** A specific repo/codebase? A general-purpose assistant chat? Or should this group be registered as a **project** — the place where spun-out peer workspaces get their own chats (Telegram: topics in this forum group)?
   - To register a project: confirm Topics are enabled and the bot is admin with Manage Topics, ask which workspaces route here (or whether it's the default), then call `register_project` followed by `task_complete`. Skip the remaining steps.
